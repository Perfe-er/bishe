...
class AnnoDao : BaseDao() {
    ...
    suspend fun pubAnno(call: ApplicationCall) {
        ...
        val request = call.request
        val annoTitle = request.queryParameters["annoTitle"]//className
        val content = request.queryParameters["content"]//className
        val releaseID = request.queryParameters["releaseID"]//className
        val releDate = request.queryParameters["releDate"]//className
        val classIDs = request.queryParameters["classIDs"]//接收班级id classIDs  ->  "1,2,3,4,7" 中间用,分割
        val anno = Anno()
    ...
    suspend fun deleteAnno(call: ApplicationCall) {
        ...
        val request = call.request
        val annoID = request.queryParameters["annoID"]?.toInt()
        val anno = Anno()
