...
class SignDao : BaseDao() {
    ...
    suspend fun getSignedUser(call: ApplicationCall) {
        ...
        val recClass = JdbcConnection.bootstrap.queryTable(SignReceive::class.java)
            .addCondition { c -> c.add(C.eq("signID", sign.signID)) }
            .list(SignReceive::class.java)
    ...
    suspend fun endSign(call: ApplicationCall){
        ...
        JdbcConnection.bootstrap.query(sign).setFields("endDate","signType").update()
        writeGsonResponds(JSON.toJSONString(HttpResult<Sign>(sign, 200, "结束签到")), call)
    }
...
//        sign.endDate = endDate;
        ...
        writeGsonResponds(JSON.toJSONString(HttpResult<Sign>(sign, 200, "发布成功")), call)
